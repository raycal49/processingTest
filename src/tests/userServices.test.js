...
describe('User Service', async () => {
    // these test selectPlan which accepts the arguments: (userId, planName, cardLast4)
    // findActivePlanByName is called first in selectPlan and accepts planName and returns: (plan_id, price_per_month)
    // findActiveSubscription is called second in selectPlan and accepts userId and returns: (subscription_id, plan_id, started_at)
    // 
    // subscribeToPlan is called third, if the user has no active sub, takes in userId, planId, princePerMonth, cardLast4, 
    // and returns (subscription_id, started_at)

    // changePlan is called last and accepts userId, planId, pricePerMonth, cardLast4 and returns subscription_id, started_at
